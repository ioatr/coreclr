...
int initAssembly(
            ...
            const char* clrFilesAbsolutePath,
            const char* managedAssemblyAbsolutePath,
	    QBATTLE_INIT_FUNC* init,
	    QBATTLE_START_FUNC* start,
            QBATTLE_UPDATE_FUNC* update,
            QBATTLE_FREE_HGLOBAL_FUNC* freeHGlobal)
{
    // Indicates failure
    int exitCode = -1;
	...
	coreclr_create_delegate_ptr createDelegate = (coreclr_create_delegate_ptr)dlsym(coreclrLib, "coreclr_create_delegate");
        ...
        else
        {
            ...
            {
		st = createDelegate(hostHandle, domainId, "qbattle", "qbattle.Program", "Init", (void**)init);
		st = createDelegate(hostHandle, domainId, "qbattle", "qbattle.Program", "StartBattle", (void**)start);
		st = createDelegate(hostHandle, domainId, "qbattle", "qbattle.Program", "UpdateBattle", (void**)update);
		st = createDelegate(hostHandle, domainId, "qbattle", "qbattle.Program", "FreeHGlobal", (void**)freeHGlobal);
            }
        }
